...
value of $x_n$. Thus, each value of $x_n$ is either $0,1, \ldots, m-1$ and the quantity $x_n / m$ is Pseudorandom number\footnote{If we know the initial seed then we can calculate the whole samples so these numbers are not really a random numbers so we call these as \textbf{Pseudorandom numbers.} } and follows
